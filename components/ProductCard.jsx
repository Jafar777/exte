...
const ProductCard = ({ product }) => {
    ...
    const [imageLoading, setImageLoading] = useState(true); // Add this line

    const fallbackImage = '/images/placeholder-product.jpg';

    const handleImageError = () => {
        setImageError(true);
        setImageLoading(false); // Use imageLoading here
    };
    ...
    <CiHeart className="w-4 h-4 text-gray-600" />
                    ...
                    {product.sizes && product.sizes.every(size => size.stock === 0) && (
                        <div className="absolute inset-0 bg-white bg-opacity-80 flex items-center justify-center">
                            <span className="text-gray-900 font-light tracking-wide text-sm">OUT OF STOCK</span>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-2">
                    ...
                    {product.sizes && product.sizes.some(size => size.stock > 0) && (
                        <div className="flex items-center space-x-1">
                            <span className="text-xs text-gray-500 font-light">Sizes:</span>
                            <div className="flex space-x-1">
                                ...
                                {product.sizes.filter(size => size.stock > 0).length > 3 && (
                                    <span className="text-xs text-gray-500 font-light">+ more</span>
                                )}
                            </div>
                        </div>
                    )}
